...
### 🚨 Errores detectados en Auditoría Técnica (16/05/2026)

- [ ] **Middleware:** El estado de la sala nunca se actualiza a `JUGANDO`. En `SocketService.java`, se cambia a `PREPARANDO` pero no hay transición a `JUGANDO` cuando comienza el combate, lo que afecta a la visualización en la lista de salas.
- [ ] **Seguridad Frontend:** Las credenciales de administrador del middleware (`middleware_admin` / `clave_secreta_del_middleware_2026`) están hardcodeadas en `auth.service.ts`. Deberían manejarse de forma más segura o eliminarse si no son estrictamente necesarias en el cliente.
- [ ] **Frontend - Cabeceras ausentes:** El `RoomService` realiza peticiones al Middleware (como `createRoom` y `isSalaActiva`) sin incluir las cabeceras `X-Fingerprint` y `Authorization`. Dado que el `JwtFilter` del Middleware las exige, estas peticiones fallarán con un error 401.
- [ ] **Arquitectura Frontend:** El método `withMiddlewareToken` en `AuthService` realiza un login contra el Middleware en cada llamada protegida. Esto es ineficiente; se debería cachear el token del middleware mientras sea válido.
- [x] **Documentación:** El archivo `explicacion_comunicacion.md` indica puertos incorrectos (ej. API en 8080) que contradicen la configuración real de `application.properties` (API en 8081) y `docker-compose.yml`. (Resuelto 16/05/2026)
- [ ] **Maven/Spring Boot:** Los archivos `pom.xml` utilizan versiones de Spring Boot inexistentes (4.0.2 y 4.0.6). Esto puede causar problemas de resolución de dependencias y plugins en entornos de compilación limpios.
- [ ] **Consistencia de Errores:** El `AuthController` del Middleware devuelve strings planos (ej. "USUARIO_NO_ENCONTRADO") en caso de error, mientras que la API devuelve objetos JSON. El Frontend espera una estructura consistente para mostrar mensajes de error al usuario.
- [ ] **Validación de Unión:** Aunque el servidor bloquea la unión a salas no disponibles, el botón "Unirse" en el frontend debería deshabilitarse explícitamente para estados distintos a `ESPERANDO` para mejorar la UX.
