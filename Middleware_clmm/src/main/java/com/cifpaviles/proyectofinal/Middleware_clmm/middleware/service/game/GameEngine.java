 ...
 * - PAS_ARA (Aranessa): 20% de esquivar un impacto entrante (Tripulación de los
 * Muertos)
 * - PAS_IKT (Ikit): 20% de no consumir cooldown al usar habilidad ofensiva
 * - PAS_GEL (Gelt): al impactar un barco, reduce cooldown de habilidad activa
 * aleatoria
 *
...
public class GameEngine {
    ...
    public void setBackendClient(
            com.cifpaviles.proyectofinal.Middleware_clmm.middleware.service.impl.BackendClient client) {
    ...
    public void procesarDisparo(String jugadorId, int x, int y) {
        ...
        if (atacante.isHaAtacadoEsteTurno() && !atacante.isTurnoExtraWulfrik())
            return;
        ...
        // pero se tratan como BARCO en el impacto real (el estado es simplemente
        // informativo).

        // Consumir el flag de turno extra si procede
        boolean eraDisparoExtra = atacante.isTurnoExtraWulfrik();
        if (eraDisparoExtra)
            atacante.setTurnoExtraWulfrik(false);
    ...
    public void usarHabilidad(String jugadorId, String habilidadId, int x, int y) {
        ...
        if (habilidad != null && habilidad.estaLista()) {
            ...
            // Ikit Claw PAS_IKT: 20% de probabilidad de no consumir enfriamiento en
            // habilidades OFENSIVAS
            boolean bypassCooldown = habilidad.getTipo() == SkillType.ACTIVA_OFENSIVA
            ...
            if (!bypassCooldown) {
                ...
                state.setMensajeEstado(
                        state.getMensajeEstado() + " ¡Ingenio Skaven! La habilidad no ha consumido enfriamiento.");
            }
            ...
            if (habilidad.getTipo() == SkillType.ACTIVA_OFENSIVA) {
                state.cambiarTurno();
            }
        }
        verificarVictoria();
    ...
    private void ejecutarEfectoHabilidad(Skill skill, Player owner, int x, int y) {
        ...
        switch (skill.getId()) {
            // --- Wulfrik ---
            case "SKL_WUL_1":
                ejecutarDesafioErrante(owner, enemigo, x, y);
                break;
            case "SKL_WUL_2":
                ejecutarColmilloMares(owner, enemigo, x, y);
                break;
            case "SKL_WUL_3":
                ejecutarFavorRuinoso(owner);
                break;
            // --- Aislinn ---
            case "SKL_AIS_1":
                ejecutarCorteLothern(owner, enemigo);
                break;
            case "SKL_AIS_2":
                ejecutarIraMathlann(owner, enemigo, x, y);
                break;
            case "SKL_AIS_3":
                ejecutarBrumaMarina(owner, x, y);
                break;
            // --- Lokhir ---
            case "SKL_LOK_1":
                ejecutarAndanadaDruchii(owner, enemigo, x, y);
                break;
            case "SKL_LOK_2":
                ejecutarFuriaCorsaria(enemigo, x, y);
                break;
            case "SKL_LOK_3":
                ejecutarYelmoKraken(owner);
                break;
            case "SKL_LOK_4":
                ejecutarVenganzaKarondKar(owner, enemigo);
                break;
            // --- Aranessa ---
            case "SKL_ARA_1":
                ejecutarPolvoraVampirica(owner, enemigo, x, y);
                break;
            case "SKL_ARA_2":
                ejecutarDisparoSaloma(owner, enemigo, x, y);
                break;
            case "SKL_ARA_3":
                ejecutarHijaStromfels(owner);
                break;
            // --- Ikit Claw ---
            case "SKL_IKT_1":
                ejecutarRayoBrujo(owner, enemigo, x, y);
                break;
            case "SKL_IKT_2":
                ejecutarCoheteMuerte(owner, enemigo, x, y);
                break;
            case "SKL_IKT_3":
                ejecutarEscudoEnergiaBruja(owner, x, y);
                break;
            // --- Balthasar Gelt ---
            case "SKL_GEL_1":
                ejecutarTransmutacionPlomo(owner, enemigo, x, y);
                break;
            case "SKL_GEL_2":
                ejecutarLluviaMetal(owner, enemigo);
                break;
            case "SKL_GEL_3":
                ejecutarCuerpoHierro(owner);
                break;
            default:
                state.setMensajeEstado("Habilidad desconocida: " + skill.getId());
        }
    }
    ...
    /**
     * SKL_WUL_1: Dispara a (x,y); si falla, revela la posicion de un BARCO enemigo
     * aleatorio.
     */
    private void ejecutarDesafioErrante(Player owner, Player enemigo, int x, int y) {
        ...
        if (postImpacto == CellStatus.AGUA_GOLPEADA) {
            ...
            if (!barcos.isEmpty()) {
                ...
                state.setMensajeEstado("¡Desafío del Errante! Tras fallar el tiro, " + owner.getNombre()
                        + " ha avistado un barco enemigo.");
            } else {
    ...
    private void ejecutarFavorRuinoso(Player owner) {
        ...
        if (celdas.isEmpty()) {
            state.setMensajeEstado("No quedan casillas para proteger.");
            return;
        }
        ...
        state.setMensajeEstado(
                "¡Favor Ruinoso! Los Dioses del Caos han protegido una de las casillas de " + owner.getNombre() + ".");
    }
    ...
    private void ejecutarBrumaMarina(Player owner, int x, int y) {
        ...
        state.setMensajeEstado("¡Bruma Marina! Una densa niebla mágica oculta y protege un sector de la flota de "
                + owner.getNombre() + ".");
    }
    ...
    /**
     * SKL_LOK_2: Revela (marca como REVELADA en tablero) las posiciones de BARCO en
     * area 3x3 centrada en (x,y).
     */
    private void ejecutarFuriaCorsaria(Player enemigo, int x, int y) {
        ...
        state.setMensajeEstado(
                encontrado ? "¡Furia Corsaria! Se han avistado barcos enemigos en el área de exploración."
                        : "¡Furia Corsaria! El área parece estar despejada de enemigos.");
    }

    /**
     * SKL_LOK_3: Yelmo del Kraken.
     * Escuda completamente el barco más grande del jugador (el Arca Negra).
     * El escudo es frágil: si una casilla recibe un impacto, todos los escudos del
     * Arca desaparecen.
     */
    private void ejecutarYelmoKraken(Player owner) {
        ...
        if (escudosAplicados > 0) {
            state.setMensajeEstado(
                    "¡Yelmo del Kraken! El navío insignia de Lokhir ha desplegado sus barreras místicas.");
        } else {
    ...
    /**
     * Activa el modo venganza de Lokhir reemplazando su defensa por un bombardeo
     * ofensivo.
     */
    private void activarVenganzaLokhir(Player owner) {
        ...
        state.setMensajeEstado(state.getMensajeEstado()
                + " ¡Lokhir entra en frenesí! El Arca Negra ha caído, pero su furia se desata: Yelmo del Kraken ha sido reemplazado por Venganza de Karond Kar.");
    }
    ...
    /**
     * SKL_ARA_1: Dispara a (x,y); si es BARCO, el fuego se propaga a las 4 casillas
     * adyacentes.
     */
        ...
        if (res.contains("Impacto") || res.contains("HUNDIDO")) {
            ...
            for (int[] a : adj) {
                ...
                if (nx >= 0 && nx < 10 && ny >= 0 && ny < 10) {
                    ...
                    if (rExtra.contains("Impacto") || rExtra.contains("HUNDIDO"))
                        impactosExtra++;
                }
            }
    ...
    private void ejecutarDisparoSaloma(Player owner, Player enemigo, int x, int y) {
        ...
        msg.append(impactos > 0 ? "¡Y ha causado estragos en la flota!"
                : "El impacto ha levantado una columna de agua gigante.");
        state.setMensajeEstado(msg.toString());
    }
    ...
    private void ejecutarHijaStromfels(Player owner) {
        ...
        state.setMensajeEstado("¡Hija de Stromfels! La bendición del Dios del Mar hace invulnerable la flota de "
                + owner.getNombre() + ".");
    }
    ...
    private void ejecutarTransmutacionPlomo(Player owner, Player enemigo, int x, int y) {
        StringBuilder msg = new StringBuilder(
                "¡Transmutación de Plomo! El aire se vuelve pesado y el mar se torna dorado. ");
        int impactos = 0;
        for (int dx = 0; dx <= 1; dx++) {
            for (int dy = 0; dy <= 1; dy++) {
                int nx = x + dx, ny = y + dy;
        ...
        msg.append(impactos > 0 ? "¡Varios barcos han sido atrapados por la alquimia!"
                : "La transmutación solo ha afectado al agua.");
        state.setMensajeEstado(msg.toString());
    }
    ...
    private void ejecutarLluviaMetal(Player owner, Player enemigo) {
        ...
        state.setMensajeEstado("¡Lluvia de Metal! Proyectiles dorados caen del cielo. "
                + (impactos > 0 ? "Se han registrado " + impactos + " impactos."
                        : "Los proyectiles se hundieron en el mar."));
    }
    ...
    /**
     * Aplica daño en (nx,ny) para habilidades activas (respeta escudos y
     * colisiones).
    ...
    /**
     * Lokhir PAS_LOK: tras hundir un barco en (x,y), revela una celda BARCO
     * adyacente
    ...
    private boolean dfsSunkCheck(CellStatus[][] tablero, int x, int y, boolean[] visited) {
        ...
        visited[idx] = true;
        CellStatus s = tablero[x][y];
        if (s == CellStatus.AGUA || s == CellStatus.AGUA_GOLPEADA || s == CellStatus.HUNDIDO)
            return true;
        // BARCO y REVELADA son celdas de barco intactas: el barco aún no está hundido
        if (s == CellStatus.BARCO || s == CellStatus.REVELADA)
            return false;
    ...
    private void finalizarJuego(String ganadorId) {
        ...
        if (backendClient != null && !state.isStatsGuardadas()) {
            ...
            guardarEstadisticas(ganadorId);
        }
    }

    private void guardarEstadisticas(String ganadorId) {
        try {
